...
class WalletDb extends BaseStore {
  ...
  checkNextGeneratedKey() {
    ...
    // locked
    if (!this.state.wallet.encrypted_brainkey) {
      return;
    }
  ...
  process_transaction(tr, signer_pubkeys, broadcast) {
    ...
    return WalletUnlockActions.unlock().then(() => {
      return tr.set_required_fees().then(() => {
        ...
        return tr.get_potential_signatures().then(({
          pubkeys,
          addys
        }) => {
  ...
  onCreateWallet(
    ...
    let walletCreateFct = (dictionary) => {
      return new Promise((resolve) => {
        ...
        // encryption_key is the global encryption key
        // (does not change even if the passsword changes)
  ...
  generateNextKey(save = true) {
    ...
    private_key = key.get_brainPrivateKey(brainkey, sequence);
  ...
  loadDbData() {
    console.log('old wallet state', this.state);
