...
// Setting this to BTS to prevent loading issues when used with BTS chain which is the most
// usual case currently
const CORE_ASSET = 'TESTPLAYS';
...
class SettingsStore {
  constructor() {
    ...
    //   "OPEN.BTC", "OPEN.ETH", "OPEN.USDT", "OPEN.EURT", CORE_ASSET
    // ]);
  ...
  onAddStarMarket(market) {
    ...
    if (!this.starredMarkets.has(marketID)) {
      this.starredMarkets = this.starredMarkets.set(
        marketID,
        {quote: market.quote, base: market.base}
      );
