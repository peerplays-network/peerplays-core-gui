...
function getPrice(coreAsset, currentAsset) {
  let coreAmount = currentAsset.options.core_exchange_rate.base.amount
    / Math.pow(10, coreAsset.precision);
  let currentAmount = currentAsset.options.core_exchange_rate.quote.amount
    / Math.pow(10, currentAsset.precision);
        ...
        let fees = Object.values(ChainTypes.operations).map((feeIdx) => {
          ...
          for (let key in feeObj) {
            ...
            let equivalentAmount = amount
              ? `${equivalentValue(coreAsset, currentAsset, amount)} ${symbol}`
              : feeTypes['_none'];
            let equivalentAmountLTM = amount * 0.2
              ?`${equivalentValue(coreAsset, currentAsset, amount * 0.2)} ${symbol}`
              : feeTypes['_none'];
