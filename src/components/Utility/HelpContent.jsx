...
class HelpContent extends React.Component {
    ...
    setVars(str) {
      return str.replace(/(\{.+?\})/gi, (match, text) => {
        ...
        if (value.date) {
          value = utils.format_date(value.date);
        }
    ...
    render() {
      ...
      if (!value && locale !== 'en') {
        console.warn(`missing path '${this.props.path}' for locale '${locale}' help files, rolling back to 'en'`); // eslint-disable-line
        value = HelpData['en'][this.props.path];
      }

      if (!value && this.props.alt_path) {
        console.warn(`missing path '${this.props.path}' for locale '${locale}' help files, rolling back to alt_path '${this.props.alt_path}'`); // eslint-disable-line
        value = HelpData[locale][this.props.alt_path];
      }

      if (!value && this.props.alt_path && locale !== 'en') {
        console.warn(`missing alt_path '${this.props.alt_path}' for locale '${locale}' help files, rolling back to 'en'`); // eslint-disable-line
