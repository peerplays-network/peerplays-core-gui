...
var Utils = {
  ...
  get_asset_price: function(quoteAmount, quoteAsset, baseAmount, baseAsset, inverted = false) {
    ...
    var price = this.get_asset_amount(quoteAmount, quoteAsset)
      / this.get_asset_amount(baseAmount, baseAsset);
    price = this.round_number(price, quoteAsset);
  ...
  format_asset: function(amount, asset, noSymbol, trailing_zeros=true) {
    ...
    return (
      `${this.format_number(amount / precision, digits, trailing_zeros)}${!noSymbol
        ? ' ' + symbol : ''}`
    );
  },
  ...
  format_price: function (
    ...
    if (inverted) {
      if (parseInt(quoteAsset.id.split('.')[2], 10) < parseInt(baseAsset.id.split('.')[2], 10)) {
        return (
          `${this.format_number((quoteAmount / precision) / (baseAmount / basePrecision),
            Math.max(5, quoteAsset.precision),trailing_zeros)}${!noSymbol
            ? '' + quoteAsset.symbol + '/' + baseAsset.symbol : ''}`
        );
      } else {
        return (
          `${this.format_number((baseAmount / basePrecision) / (quoteAmount / precision),
            Math.max(5, baseAsset.precision),trailing_zeros)}${!noSymbol
  ...
  price_to_text: function (price, base, quote, forcePrecision = null, s = null) { // eslint-disable-line
  ...
  estimateFee: function(op_type, options, globalObject) {
    ...
    let currentFees = globalObject.getIn(['parameters', 'current_fees', 'parameters', op_code, 1])
      .toJS();
    let fee = 0;
  ...
  convertPriceObj: function(fromRate, toRate, fromID, toID) {
    ...
    if (this.is_object_type(toRate.id, 'asset')) {
      toID = toRate.id;
      toRate = toRate.bitasset
        ? toRate.bitasset.current_feed.settlement_price
        : toRate.options.core_exchange_rate;
    }
  ...
  convertPrice: function(fromRate, toRate, fromID, toID) {
    ...
    if (fromRate.toJS && this.is_object_type(fromRate.get('id'), 'asset')) {
      ...
      fromRate = fromRate.get('bitasset')
        ? fromRate.getIn(['bitasset', 'current_feed', 'settlement_price']).toJS()
    ...
    if (toRate.toJS && this.is_object_type(toRate.get('id'), 'asset')) {
      ...
      toRate = toRate.get('bitasset')
        ? toRate.getIn(['bitasset', 'current_feed', 'settlement_price']).toJS()
  ...
  convertValueObj: function(priceObject, amount, fromAsset, toAsset) {
    ...
    let eqValue = fromAsset.id !== toAsset.id ?
  ...
  convertValue: function(priceObject, amount, fromAsset, toAsset) {
    ...
    let quotePrecision = this.get_asset_precision(fromAsset.toJS
      ...
      : fromAsset.precision);
    let basePrecision = this.get_asset_precision(toAsset.toJS
      ? toAsset.get('precision')
      : toAsset.precision);
    let assetPrice = this.get_asset_price(
      priceObject.quote.amount, fromAsset, priceObject.base.amount, toAsset
    );
    let eqValue = (
      fromAsset.toJS
        ? fromAsset.get('id')
  ...
  replaceName(name, isBitAsset = false) {
    ...
    return {
      name: asset_utils.getSymbol(name),
