...
function lookupAccounts(startChar, limit = 1) {
  return Apis.instance()
    ...
    .then((accounts) => {
      let newInput = accounts.map((account) => account[0]);
      return Apis.instance()
        .db_api()
          ...
          lookupAccounts(startChar, accountLimit).then((accounts) => {
            accounts.forEach((account) => {
              if ('balances' in account[1]) {
                for (let i = 0; i < account[1].balances.length; i++) {
                  if (
                    account[1].balances[i].asset_type === assetID
                    && parseInt(account[1].balances[i].balance, 10) > 0
                  ) {
                    // console.log(account[1].account.name, "has OBITS");
                    holders[account[1].account.name] = account[1].balances[i].balance;
                    break;
                  }
                }
              }
            ...
            if (accounts.length === accountLimit && iterations < maxIter) {
              iterations++;
              console.log(
              ...
              return balanceCheck(accounts[accounts.length - 1][1].account.name);
            }

            // DONE, save holders to a file here
            console.log('Found', Object.keys(holders).length, 'OBITS holders');
          });
        } catch (err) {
          console.log('err:', err);
        }
