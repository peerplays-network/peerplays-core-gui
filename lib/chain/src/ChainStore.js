...
var {object_type,impl_object_type} = require( "./ChainTypes");
...
class ChainStore
{
   ...
   clearCache() {
      ...
      // tournaments_by_state is a map of state_string => Set of tournament objects 
      this.tournaments_by_state = Immutable.Map()

      this.tournaments_by_account   = Immutable.Map()
      this.active_tournaments_by_account = Immutable.Map()
      this.upcoming_tournaments_by_account = Immutable.Map()
      this.upcoming_tournaments = null;
      this.active_tournaments = null;
   }
    ...
    */
   getTournamentsInState(stateString, apiCall)
   {
      let upcoming_tournaments_set = this.tournaments_by_state.get(stateString);
      if (upcoming_tournaments_set !== undefined)
         return upcoming_tournaments_set;

      // HACK until I push out the next version of the witness node
      let params = stateString == "accepting_registrations" ? [null, 100] : [100];

      Apis.instance().db_api().exec( apiCall, params )
         .then( tournaments => {
            let tournamentsByState = {};
            for( let i = 0; i < tournaments.length; ++i )
            {
               let newTournamentObject = this._updateObject( tournaments[i] );
               let state = newTournamentObject.get("state");
               if (state in tournamentsByState)
                  tournamentsByState[state].push(newTournamentObject);
               else
                  tournamentsByState[state] = [newTournamentObject];
            }
            if (!(stateString in tournamentsByState))
               tournamentsByState[stateString] = [];
            for (let state in tournamentsByState)
               this.tournaments_by_state = this.tournaments_by_state.set(state, 
                                                                         Immutable.Set(tournamentsByState[state]));
            this.notifySubscribers();
         });
      return Immutable.Set();
   }
   
   getAccountUpcomingTournaments(upcomingTournaments, idAccount)
   {
	    upcomingTournaments = upcomingTournaments.toJS();
		var list = [];
		for (var i = 0; i < upcomingTournaments.length; i++) {
			if (this.getObject(upcomingTournaments[i].tournament_details_id) !== undefined) {
				if (this.getObject(upcomingTournaments[i].tournament_details_id).get("registered_players").includes(idAccount)) {
					list.push(upcomingTournaments[i]);
				}
			}
		}
   ...
   {
      return this.getTournamentsInState("accepting_registrations", "get_upcoming_tournaments");
   }

   getTournamentsAwaitingStart()
   {
      return this.getTournamentsInState("awaiting_start", "get_active_tournaments");
   }

   getTournamentsInProgress()
   {
      return this.getTournamentsInState("in_progress", "get_active_tournaments");
   }

   fetchActiveTournaments()
   {
      return new Promise( (resolve,reject ) => {
         Apis.instance().db_api().exec( "get_active_tournaments", [ 100 ] )
            .then( tournaments => {
               let list = [];
               for( let i = 0; i < tournaments.length; ++i )
               {
                  this._updateObject( tournaments[i] );
                  list.push(tournaments[i].id);
               }
               let activeTournamentsList = Immutable.List(list);
               this.active_tournaments_by_account = this.active_tournaments_by_account.set( null, activeTournamentsList );
               this.notifySubscribers();
               resolve(activeTournamentsList);
            },
            error => {
               this.active_tournaments_by_account = this.active_tournaments_by_account.delete( null );
               this.resolve(null);
            }, reject ) } );
   }

   fetchUpcomingTournaments(account_id)
   {
      return new Promise( (resolve,reject ) => {
         Apis.instance().db_api().exec( "get_upcoming_tournaments", [ account_id, 100 ] )
            .then( tournaments => {
               let list = [];
               for( let i = 0; i < tournaments.length; ++i )
               {
               ...
               let upcomingTournamentsList = Immutable.List(list);
               this.tournaments_by_account = this.tournaments_by_account.set( account_id, upcomingTournamentsList );
               this.notifySubscribers();
               resolve(upcomingTournamentsList);
            },
            error => {
               this.tournaments_by_account = this.tournaments_by_account.delete( account_id );
               this.resolve(null);
            }, reject ) } );
   }
   ...
   _updateObject( object, notify_subscribers, emit = true )
   {
      ...
      // TOURNAMENT OBJECT
        ...
        let newObject = Immutable.fromJS(object);
        let priorState = prior.get("state");
        let newState = newObject.get("state");
        if (priorState !== newState)
        {
           let tournaments_in_old_state = this.tournaments_by_state.get(priorState);
           if (tournaments_in_old_state)
              this.tournaments_by_state = this.tournaments_by_state.set(priorState, 
                                                                        tournaments_in_old_state.delete(prior));

           let tournaments_in_new_state = this.tournaments_by_state.get(newState);
           if (tournaments_in_new_state)
              this.tournaments_by_state = this.tournaments_by_state.set(newState, 
                                                                        tournaments_in_new_state.add(newObject));
        }
        prior = newObject;
      }
