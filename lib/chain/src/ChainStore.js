...
class ChainStore
{
   ...
   getAccountTournaments(tournaments, idAccount)
   {
	    tournaments = tournaments.toJS();
		var list = [];
		for (var i = 0; i < tournaments.length; i++) {
			if (this.getObject(tournaments[i].tournament_details_id) !== undefined) {
				if (this.getObject(tournaments[i].tournament_details_id).get("registered_players").includes(idAccount)) {
					list.push(tournaments[i]);
				}
			}
		}
