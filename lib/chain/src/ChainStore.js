...
class ChainStore {
            ...
            return db_api.exec("get_objects", [["2.1.0"]]).then(optional_objects => {
                ...
                for(let i = 0; i < optional_objects.length; i++) {
                    ...
                    if (optional_object) {
                        ...
                        this.chain_time_offset.push((new Date).getTime() - this.timeStringToDate(optional_object.time).getTime());
   ...
   getSimpleObjectById(id) {
        return new Promise((success, fail) => {
            if(!ChainValidation.is_object_id(id)) {
                fail("argument is not an object id: " + JSON.stringify(id));
            }
            ...
            Apis.instance().db_api().exec("get_objects", [[id]]).then(objects => {
   ...
   getBalanceObjects(address) {
    ...
    getTournamentIdsInState(accountId, stateString) {
        let tournamentIdsForThisAccountAndState = undefined;
        ...
        Apis.instance().db_api().exec("get_tournaments_in_state", [stateString, 100])
            .then(tournaments => {
                let originalTournamentIdsInState = this.tournament_ids_by_state.getIn([accountId, stateString]);
                ...
                if (this.tournament_ids_by_state.getIn([accountId, stateString]) != originalTournamentIdsInState) {
                    this.notifySubscribers();
                }
            });
        return tournamentIdsForThisAccountAndState;
    ...
    getRegisteredTournamentIds(accountId) {
        ...
        Apis.instance().db_api().exec("get_registered_tournaments", [accountId, 100])
            .then(tournamentIds => {
                ...
                let newTournamentIds = new Immutable.Set(tournamentIds);
                if(!originalTournamentIds.equals(newTournamentIds)) {
                    this.registered_tournament_ids_by_player = this.registered_tournament_ids_by_player.set(accountId, newTournamentIds);
                    this.notifySubscribers();
                }
            });
    ...
    getWitnessAccount(witness_id) {
        return new Promise((success, fail) => {
            ...
            this.getSimpleObjectById(witness_id).then(witness => {
                this.getSimpleObjectById(witness.witness_account).then(account => {
                    this.account_by_witness_id = this.account_by_witness_id.set(witness_id, account);
                    success(account);
                });
            });
          ...
          Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true])
              .then(results => {
                 ...
                 account.proposals = new Immutable.Set();
                  ...
                  account.pending_dividend_payments = account.pending_dividend_payments.withMutations(set => {
                ...
                if (sub_to_objects.length) {
                    Apis.instance().db_api().exec("get_objects", [sub_to_objects]);
                }
   ...
   /**
    *  @return a promise with a list that contains all available Sport objects in the Blockchain
    */
   ...
   /**
    *  @param sportId - A Graphene object Id of a Sport object
    *  @return Return a list of all event groups for a sport (e.g. all soccer leagues in soccer)
    */
   ...
   getEventGroupsList(sportId) {
     ...
     if (eventGroupsList === undefined) {
       ...
       this.event_groups_list_by_sport_id = this.event_groups_list_by_sport_id.set( sportId, Immutable.Set() );

       Apis.instance().db_api().exec( "list_event_groups", [sportId] )
           .then( eventGroupsList => {
             let set = new Set();
             for( let i = 0; i < eventGroupsList.length; ++i ) {
               set.add(eventGroupsList[i]);
             }
             this.event_groups_list_by_sport_id = this.event_groups_list_by_sport_id.set( sportId, Immutable.Set(set) );
             this.notifySubscribers();
           },
           error => {
   ...
   /**
    *  @param eventId - A Graphene object Id of a Event object
    *  @return Return a list of all betting market groups for an event
    */
   ...
   getBettingMarketGroupsList(eventId) {
     ...
     if (bettingMarketGroupsList === undefined) {
       ...
       this.betting_market_groups_list_by_sport_id = this.betting_market_groups_list_by_sport_id.set( eventId, Immutable.Set() );

       Apis.instance().db_api().exec( "list_betting_market_groups", [eventId] )
           .then( bettingMarketGroupsList => {
             let set = new Set();
             for( let i = 0; i < bettingMarketGroupsList.length; ++i ) {
               set.add(bettingMarketGroupsList[i]);
             }
             this.betting_market_groups_list_by_sport_id = this.betting_market_groups_list_by_sport_id.set( eventId, Immutable.Set(set) );
             this.notifySubscribers();
           },
           error => {
             this.betting_market_groups_list_by_sport_id = this.betting_market_groups_list_by_sport_id.delete( eventId );
           });
     }
   ...
   /**
    *  @param bettingMarketGroupId - A Graphene object Id of a betting market group object
    *  @return Return a list of all betting markets for a betting market group
    */
   ...
   getBettingMarketsList(bettingMarketGroupId) {
     ...
     if (bettingMarketsList === undefined) {
       ...
       this.betting_markets_list_by_sport_id = this.betting_markets_list_by_sport_id.set( bettingMarketGroupId, Immutable.Set() );

       Apis.instance().db_api().exec( "list_betting_markets", [bettingMarketGroupId] )
           .then( bettingMarketsList => {
             let set = new Set();
             for( let i = 0; i < bettingMarketsList.length; ++i ) {
               set.add(bettingMarketsList[i]);
             }
             this.betting_markets_list_by_sport_id = this.betting_markets_list_by_sport_id.set( bettingMarketGroupId, Immutable.Set(set) );
             this.notifySubscribers();
           },
           error => {
             this.betting_markets_list_by_sport_id = this.betting_markets_list_by_sport_id.delete( bettingMarketGroupId );
           });
     }
   ...
   /**
    * @return a promise with the betting statistics object
    */
    ...
    */
    ...
    if (object.id.substring(0,transaction_prefix.length) == transaction_prefix) {
    ...
    } else if (object.id.substring(0,budget_record_prefix.length) == budget_record_prefix) {
        return; // console.log("not interested in budget record:", object);
      ...
      // TOURNAMENT OBJECT
        ...
        if (priorState !== newState) {
            this.tournament_ids_by_state = this.tournament_ids_by_state.map((stateMap, accountId) => {
                return stateMap.map((tournamentIdSet, stateString) => {
                    if (stateString === priorState) {
                        return tournamentIdSet.remove(object.id);
                    } else if (stateString === newState && 
                        (accountId === null ||
        ...
        if (priorRegisteredPlayers != newRegisteredPlayers) {
            this.registered_tournament_ids_by_player = this.registered_tournament_ids_by_player.map((tournamentIdsSet, accountId) => {
   ...
   setLastTournamentId(current_tournament_id) {
        if(current_tournament_id === null) {
            ...
            let current_tournament_id_short = parseFloat(current_tournament_id.substring(tournament_prefix.length)),
    ...
    getTournaments(last_tournament_id, limit = 5, start_tournament_id) {
        ...
        return Apis.instance().db_api().exec("get_tournaments", [last_tournament_id, limit, start_tournament_id]).then((tournaments) => {
    ...
    getLastTournamentId() {
        return new Promise((resolve, reject) => {
            if(this.last_tournament_id === undefined) {
                ...
                Apis.instance().db_api().exec("get_tournaments", [tournament_prefix + '0', 1, tournament_prefix + '0']).then((tournaments) => {
   ...
   getObjectsByVoteIds(vote_ids) {
    ...
    __getBlocksForScan(lastBlock) {
        ...
        return new Promise((success, fail) => {
            ...
            db_api.exec("get_dynamic_global_properties", []).then(globalProperties => {
                lastBlock = this.last_processed_block = globalProperties['head_block_number'];
                        ...
                        block.transactions.forEach(tx =>
                            tx.operations.forEach(op => {
                                ...
                                if(!isInit) {
                                    ...
                                    if(this.recent_operations.size >= operations_stack_size && this.recent_blocks.size >= block_stack_size) {
    ...
    fetchRecentOperations(lastBlock = null) {
        ...
        this.__getBlocksForScan(lastBlock).then(({lastBlock, scanToBlock}) => {
            this.__bindBlock(lastBlock, scanToBlock, isInit).then(() => {
                if(isInit) {
