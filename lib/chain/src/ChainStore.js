...
let {Apis} = require("peerplaysjs-ws");
...
class ChainStore {
	...
	async getSimpleObjectById(id) {
		...
		let objects = await Apis.instance().db_api().exec("get_objects", [[id]]);
		...
		Apis.instance().db_api().exec("get_tournaments_in_state", [stateString, 100])
			.then(tournaments => {
				...
				tournaments.forEach(tournament => {
					...
					this._updateObject(tournament, false); // false = don't notify on change
				});
	 ...
	 */
	async getWitnessAccount(witness_id) {
		let account = this.account_by_witness_id.get(witness_id);
		
		if(account)
			return account;
		
		let witness = await this.getSimpleObjectById(witness_id);
		account = await this.getSimpleObjectById(witness.witness_account);
				...
				.then(results => {
					...
					account.pending_dividend_payments = account.pending_dividend_payments.withMutations(set => {
						pending_dividend_payments.forEach(payments => {
							this._updateObject(payments, false)
							set.add(payments)
						});
					});
				...
				list = list.withMutations(l => {
					tournaments.forEach((tournament) => {
						...
						if(!this.objects_by_id.has(tournament.id)) {
							this._updateObject(tournament, false);
						}
	...
	async fetchRecentOperations(lastBlock = null) {
		...
		if(!lastBlock) {
			let globalProperties = await db_api.exec("get_dynamic_global_properties", []);
		...
		while(lastBlock > scanToBlock) {
			let block = await db_api.exec("get_block", [lastBlock]);
			block.id = lastBlock;
			if(typeof block.timestamp === "string") block.timestamp += "+00:00";
			block.timestamp = new Date(block.timestamp);
			let witness = await this.getWitnessAccount(block.witness);
